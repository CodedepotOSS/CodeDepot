...
class CSharpLexer(RegexLexer):
    ...
    for levelname, cs_ident in levels.items():
        tokens[levelname] = {
            'root': [
                (r'^([ \t]*)(throw|return)([ \t]*)', bygroups(Text, Keyword, Text)),
                # method names
                (r'^([ \t]*(?:' + cs_ident + r'(?:<[^>]*>\s*)?(?:\[\])?\s+)+?)' # return type
                ...
                (r'^([ \t]*(?:' + cs_ident + r'(?:<[^>]*>\s*)?(?:\[\])?\s+)+?)' # return type
                 ...
                 r'(\s*)(<[^>]*>)'                               # genericss
                 r'(\s*)(\()',                               # signature start
                 bygroups(using(this), Name.Function, Text, using(this), Text, Punctuation)),
                # (r'^\s*\[.*?\]', Name.Attribute),
                (r'[^\S\n]+', Text),
                ...
                (r'(abstract|as|async|await|base|break|case|catch|'
                 ...
                 r'do|else|event|explicit|extern|false|finally|'
                 r'fixed|for|foreach|goto|if|implicit|in|'
                ...
                (r'(class|struct|interface|enum)(\s+)', bygroups(Keyword, Text), 'class'),
...
class VbNetLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            (r'(?<!\.)(Class|Structure|Interface|Enum)(\s+)',
